...
<head>
    ...
    <title><?= htmlspecialchars((string)($title ?? 'Commande fournisseur')) ?></title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
</head>
<body class="bg-gray-100">
<nav class="bg-white shadow"><div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between"><h1 class="text-xl font-bold text-gray-800"><i class="fas fa-file-invoice text-blue-600 mr-3"></i>Commande fournisseur</h1><a href="/commande/dashboard" class="bg-gray-600 text-white px-4 py-2 rounded">Retour</a></div></nav>
<main class="max-w-7xl mx-auto p-6">
    ...
    <form method="POST" action="/commande/saisie" id="order-form" class="bg-white rounded-lg shadow p-6">
        <input type="hidden" name="items_json" id="items_json">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Fournisseur</label><select name="fournisseur_id" required class="w-full border rounded px-3 py-2"><option value="">Selectionner</option><?php foreach (($fournisseurs ?? []) as $f): ?><option value="<?= (int)$f['id'] ?>"><?= htmlspecialchars((string)$f['nom']) ?></option><?php endforeach; ?></select></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Reference commande</label><input type="text" name="reference_commande" placeholder="Ref-2024-001" class="w-full border rounded px-3 py-2"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Statut</label><select name="statut" class="w-full border rounded px-3 py-2"><option value="BROUILLON">Brouillon</option><option value="EN_ATTENTE">En attente</option><option value="ENVOYEE">Envoyee</option><option value="VALIDEE">Validee</option></select></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Date commande</label><input type="date" name="date_commande" value="<?= date('Y-m-d') ?>" required class="w-full border rounded px-3 py-2"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Livraison prevue</label><input type="date" name="date_livraison_prevue" class="w-full border rounded px-3 py-2"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">Remise globale (%)</label><input type="number" name="remise_globale" value="0" min="0" max="100" step="0.01" class="w-full border rounded px-3 py-2"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-2">TVA globale (%)</label><input type="number" name="tva_globale" value="0" min="0" max="100" step="0.01" class="w-full border rounded px-3 py-2"></div>
        </div>
        <div class="border rounded p-4 mb-6">
            <h2 class="font-semibold text-gray-800 mb-4">Produits</h2>
            <div class="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4">
                <select id="product" class="border rounded px-3 py-2"><option value="">Produit</option><?php foreach (($produits ?? []) as $p): ?><option value="<?= (int)$p['id'] ?>" data-price="<?= htmlspecialchars((string)$p['prix_achat']) ?>" <?= (int)($produitPreselectionneId ?? 0) === (int)$p['id'] ? 'selected' : '' ?>><?= htmlspecialchars((string)$p['nom']) ?></option><?php endforeach; ?></select>
        ...
        <button class="bg-blue-600 text-white rounded px-6 py-2"><i class="fas fa-save mr-2"></i>Enregistrer</button>
    </form>
</main>
<script>
const items = [];
...
if (product.value) product.dispatchEvent(new Event('change'));

...
foreach ($ecritures as $ecriture) {
    ...
    $totalSolde += (float)($ecriture['solde'] ?? 0);
}
...
<head>
    ...
    <title>Journal de Caisse - Gestion Pharmacie</title>
        ...
        <div class="container mx-auto px-4 py-4">
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-xl font-bold">Journal de Caisse</h1>
                        <p class="text-sm text-blue-100">Écritures comptables de caisse</p>
                    </div>
                </div>
    ...
    <main class="container mx-auto px-4 py-6">
        <section class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <p class="text-3xl font-bold <?= $totalSolde >= 0 ? 'text-green-600' : 'text-red-600' ?>">
                    <?= money($totalSolde) ?>
                </p>
            </div>
        </section>
        ...
        <section class="bg-white rounded-lg shadow-md p-6 mb-6">
            <form method="GET" action="/comptabilite/journaux/caisse" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
        ...
        <section class="bg-white rounded-lg shadow-md p-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
                ...
                <div class="space-x-2">
                    <a href="/comptabilite/exporter?type=journal&journal=CA&date_debut=<?= urlencode($dateDebut) ?>&date_fin=<?= urlencode($dateFin) ?>"
                ...
                <table class="w-full border-collapse">
                            ...
                            <?php foreach ($ecritures as $ecriture): ?>
                                <?php foreach ($ecriture['lignes'] ?? [] as $ligne): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="border border-gray-300 px-4 py-2 text-sm text-right text-green-600">
                                            <?= money($ligne['debit'] ?? 0) ?>
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2 text-sm text-right text-red-600">
                                            <?= money($ligne['credit'] ?? 0) ?>
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2 text-sm text-right font-semibold">
                                            <?= money($ecriture['solde'] ?? 0) ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                    ...
                    <tfoot>
                        <tr class="bg-gray-50 font-semibold">
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-right text-green-600"><?= money($totalDebit) ?></td>
                            <td class="border border-gray-300 px-4 py-2 text-right text-red-600"><?= money($totalCredit) ?></td>
                            <td class="border border-gray-300 px-4 py-2 text-right font-semibold"><?= money($totalSolde) ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
